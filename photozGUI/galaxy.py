...
class Galaxy():
    ...
    def __init__(self, cid, path_pattern=CAT_PATH + CAT_PATTERN):
        ...
        :param cid: cluster id (specify input/output files)
        :param path_pattern: catalog name pattern
        """
        try:
            self.data = np.loadtxt(path_pattern.format(cid))
        except IOError:
            print "\033[31m Fail to open tile {0:s} \033[0m".format(path_pattern.format(cid))
            raise IOError
    ...
    def save_reg(self, filename, gid, shape='circle', size=0.0003, color='r'):
        """
